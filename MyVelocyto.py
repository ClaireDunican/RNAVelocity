...
class VelocytoLoom:
    ...
    def perform_PCA(self, which: str="S_norm", n_components: int=None, div_by_std: bool=False) -> None:
        ...
        self.pca = PCA(n_components=n_components) #n_components experiment with 2 or 3
        if div_by_std:
        ...
        for per_variance in self.pca.explained_variance_ratio_:
            self.percentages.append(round(per_variance, 2)*100)
        #print(self.pca.explained_variance_ratio_) #[0.51368745 0.18150776 0.08213185]
        #print(np.cumsum(np.round(self.pca.explained_variance_ratio_, decimals=4)*100))

    def plot_pca(self, dim: List[int]=[0, 1, 2], elev: float=60, azim: float=-140, make_gify: bool=True, Filename: str="Original", typegroup: str="integers") -> None:
        """Plot 3d PCA
        """
        if dim == [0,1]:
            ...
            for i in range(0, len(markers)):
                ax.scatter(self.pcs[i, dim[0]], self.pcs[i, dim[1]], marker=markers[i], c=self.colorandum[i])
            hexy_codes = []
            my_keys = [k  for  k in  self.cluster_colors_dict.keys()]
            decoded_my_keys = []
            for item in my_keys:
                decoded_my_keys.append(str(item,'utf-8')) #int()
            cmap2 = cm.get_cmap('Paired')    # PiYG
            ...
            ax.set_xlabel('PC1 '+'('+str(self.percentages[0])+'%)')
            ax.set_ylabel('PC2 '+'('+str(self.percentages[1])+'%)')
            static = "PCA_plot" + Filename + ".pdf"
            plt.savefig(static)
            plt.close()
        if dim == [0, 1, 2]:
            my_markers = self.ca["Markers"]
            ...
            for i in range(0, len(markers)):
                ax.scatter(self.pcs[i, dim[0]], self.pcs[i, dim[1]], self.pcs[i, dim[2]], marker=markers[i], c=self.colorandum[i])
            #ax.scatter(self.pcs[:, dim[0]],
            #           self.pcs[:, dim[1]],
            #           self.pcs[:, dim[2]],
            #           c=self.colorandum)
            #for i, typey in enumerate(my_type):
            #    x = self.pcs[i:, dim[0]]
            #    y = self.pcs[i:, dim[1]]
            #    z = self.pcs[i:, dim[2]]
            #    plt.scatter(x, y, z, marker='x', color='red')
            #    plt.text(x+0.3, y+0.3, z+0.3, typey, fontsize=9)
            hexy_codes = []
            my_keys = [k  for  k in  self.cluster_colors_dict.keys()]
            decoded_my_keys = []
            for item in my_keys:
            ...
            ax.set_xlabel('PC1 '+'('+str(self.percentages[0])+'%)')
            ax.set_ylabel('PC2 '+'('+str(self.percentages[1])+'%)')
            ax.set_zlabel('PC3 '+'('+str(self.percentages[2])+'%)')
            static = "PCA_plot" + Filename + ".pdf"
            plt.savefig(static)
            #plt.show()
            if (make_gify == True):
    ...
    def TransitionProbabilityScatterplots(self, filename: str="SummaryTable_Mean_SelfNotRemoved_NN5_UMAP.csv", make_gify: bool=True):
        ...
        if NumberGroups == 2:
            ...
            if (NumberGroups <= 8):
                manycolours = ["#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7"]
            else:
    ...
    def basicUMAP(self, num_nei, make_gify: bool=True, distance: float=0.1, metric_method: str="euclidean", typegroup: str="integers", do3D: str="True", UMAP_3D_loc: str="/location/"):
       my_markers = self.ca["Markers"]
       mark = sorted(set(my_markers))
       corresp = ["o", "P", "D", "X", "*", "+", ">", "<"]
       ...
       if do3D == "True":
           ...
           ax = fig.add_subplot(111, projection='3d')
           for i in range(0, len(markers)):
               ax.scatter(trans.embedding_[i, 0], trans.embedding_[i, 1], trans.embedding_[i, 2], marker=markers[i], c=self.colorandum[i])
           hexy_codes = []
           my_keys = [k  for  k in  self.cluster_colors_dict.keys()]
           decoded_my_keys = []
           for item in my_keys:
           ...
           ax.set_ylabel('UMAP2')
           ax.set_zlabel('UMAP3')
           figure_name3D = 'UMAP3D_NN' + str(num_nei) + '.pdf'
    ...
    def estimate_transition_probUMAP(self, hidim: str="Sx_sz", embed: str="UMAPembed", transform: str="sqrt",ndims: int=None, n_sight: int=None, psc: float=None,knn_random: bool=False, sampled_fraction: float=0.3,sampling_probs: Tuple[float, float]=(0.5, 0.1), max_dist_embed: float=None, n_jobs: int=4, threads: int=None, calculate_randomized: bool=True, random_seed: int=15071990, **kwargs) -> None:
      ...
      if knn_random:
          ...
          embedding = getattr(self, embed) #here??????
    ...
    def calculate_embedding_shiftUMAP(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", TP_folder: str="/location/", UMAP_2D_GroupLevelTP: str="/location/") -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        def RunTransitionProbabilityCode(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling):
            r=ro.r
            r.source(path+"TransitionProbability.R")
            p=r.TransitionProbability(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling)
            return p
        #a=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, metadataToUse, UMAP_2D_GroupLevelTP, grouptype, version, Randomsampling) 
        b=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, metadataToUse, UMAP_2D_GroupLevelTP, grouptype, version, Randomsampling)
        #if second metadata feature type == True
        if manyfeatures == "True":
            version = "SampleGroup2" #secondgroup
            c=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, secondgroup, UMAP_2D_GroupLevelTP, grouptype2, version, Randomsampling) 
            d=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, secondgroup, UMAP_2D_GroupLevelTP, grouptype2, version, Randomsampling)

    def UMAPwithArrows2D(self, UMAPembedding: str="UMAPembed", UMAP_Arrows: str="MY_ARROWS_umap", numberNeighb: float=2., typegroup: str="integers", extraNNvalue: int=5):
       ...
       quiver_scale = 60
       for i in range(0, len(markers)):
           plt.scatter(self.UMAPsampleembedding[i, 0], self.UMAPsampleembedding[i, 1], marker=markers[i], c=self.colorandum[i], alpha=1, s=120, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
    ...
    def calculate_embedding_shiftUMAP3D(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", VecZ_loc: str="/location/", TP_folder: str="/location/", UMAP_3D_GroupLevelTP: str="/location/") -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        def RunTransitionProbabilityCode(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling):
            ...
            p=r.TransitionProbability(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling)
            return p
        #a=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, metadataToUse, UMAP_3D_GroupLevelTP, grouptype, version, Randomsampling) 
        b=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, metadataToUse, UMAP_3D_GroupLevelTP, grouptype, version, Randomsampling)
        #if second metadata feature type == True
        if manyfeatures == "True":
            version = "SampleGroup2" #secondgroup
            c=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, secondgroup, UMAP_3D_GroupLevelTP, grouptype2, version, Randomsampling) 
            d=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, secondgroup, UMAP_3D_GroupLevelTP, grouptype2, version, Randomsampling)
    ...
    def UMAPwithArrows3D(self, UMAPembedding: str="UMAPembed3d", UMAP_Arrows: str="MY_ARROWS_umap_3d", make_gify: bool=True, numberNeighb: float=2., typegroup: str="integers", extraNNvalue: int=5):
        #trans = umap.UMAP(n_neighbors = num_nei, n_components=3).fit(self.S_norm.T)
        UMAP_samplesembedding = getattr(self, UMAPembedding)
        ...
        for i in range(0, len(markers)):
            #ax.scatter(trans.embedding_[i, 0], trans.embedding_[i, 1], trans.embedding_[i, 2], marker=markers[i], c=self.colorandum[i])
            #plt.scatter(self.UMAPsampleembedding[i, 0], self.UMAPsampleembedding[i, 1],c="0.8", alpha=1, s=120, edgecolor="")
            ax.scatter(self.UMAPsampleembedding3d[i, 0], self.UMAPsampleembedding3d[i, 1], self.UMAPsampleembedding3d[i, 2], marker=markers[i], c=self.colorandum[i], s=35)#, alpha=1, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
            #quiver_kwargs=dict(headaxislength=7, headlength=13, headwidth=10,linewidths=0.5, width=0.005, color="black", alpha=1)
        ...
        quiver_scale = 60
        for i in range(0, len(markers)):
            plt.scatter(self.UMAPsampleembedding3d[i, 0], self.UMAPsampleembedding3d[i, 1], marker=markers[i], c=self.colorandum[i], alpha=1, s=120, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
            quiver_kwargs=dict(headaxislength=7, headlength=13, headwidth=10,linewidths=0.5, width=0.005,edgecolors="k", color="black", alpha=1, angles='xy')
        ...
        quiver_scale = 60
        for i in range(0, len(markers)):
            plt.scatter(self.UMAPsampleembedding3d[i, 0], self.UMAPsampleembedding3d[i, 2], marker=markers[i], c=self.colorandum[i], alpha=1, s=120, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
            quiver_kwargs=dict(headaxislength=7, headlength=13, headwidth=10,linewidths=0.5, width=0.005,edgecolors="k", color="black", alpha=1, angles='xy')
        ...
        quiver_scale = 60
        for i in range(0, len(markers)):
            plt.scatter(self.UMAPsampleembedding3d[i, 1], self.UMAPsampleembedding3d[i, 2], marker=markers[i], c=self.colorandum[i], alpha=1, s=120, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
            quiver_kwargs=dict(headaxislength=7, headlength=13, headwidth=10,linewidths=0.5, width=0.005,edgecolors="k", color="black", alpha=1, angles='xy')
    ...
    def calculate_velocity(self, kind: str="residual", eps: float=None, Myneighbours: float=None, VelocityFolder: str="/location/", UnsplicedFolder: str="/location/") -> None:
        ...
        file_name = "VelocityValues_NN" + str(Myneighbours) + ".csv"
        ...
        file_name = "PredictedUnspiced_NN" + str(Myneighbours) + ".csv"
    ...
    def extrapolate_cell_at_t(self, delta_t: float=1, clip: bool=True, Myneighbours: float=1, location: str="/location/") -> None:
        ...
        file_name = "Future_gene_spliced_state_correspondingtoplotsforNN" + str(Myneighbours) + ".csv"
        spliced_df.to_csv(file_name, sep=",", header=True)
    ...
    def calculate_embedding_shift(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", TP_folder: str="/location/", tSNE_GroupLevelTP: str="/location/") -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        def RunTransitionProbabilityCode(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling):
            ...
            p=r.TransitionProbability(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling)
            return p
        #a=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, metadataToUse, tSNE_GroupLevelTP, grouptype, version, Randomsampling) 
        b=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, metadataToUse, tSNE_GroupLevelTP, grouptype, version, Randomsampling)
        if manyfeatures == "True":
            version = "SampleGroup2" #secondgroup
            c=RunTransitionProbabilityCode(file_name, "TRUE", TP_folder, secondgroup, tSNE_GroupLevelTP, grouptype2, version, Randomsampling) 
            d=RunTransitionProbabilityCode(file_name, "FALSE", TP_folder, secondgroup, tSNE_GroupLevelTP, grouptype2, version, Randomsampling)
    ...
    def one_arrow(self, numberNeighb, typegroup: str="integers", extraNNvalue: int=5):
        plt.figure(None,(16,10))
        quiver_scale = 60
        my_markers = self.ca["Markers"]
        mark = sorted(set(my_markers))
        corresp = ["o", "P", "D", "1", "2", "3", "p", "D", "d", "4", "|", "-", "X", "x", "*", "+", ">", "<"]
        count = 0
        my_types = {}
        for the_type in mark:
            my_types[the_type] = corresp[count]
        ...
        for i in range(0, len(markers)):
            ...
            plt.scatter(self.embedding[i, 0], self.embedding[i, 1], marker=markers[i], c=self.colorandum[i], alpha=1, s=120, edgecolor=(0,0,0,1), lw=0.3, rasterized=True)
            quiver_kwargs=dict(headaxislength=7, headlength=13, headwidth=10,linewidths=0.5, width=0.005,edgecolors="k", color="black", alpha=1, angles='xy')
        ...
        figure_name = 'tSNE_Arrows_NN' + str(numberNeighb) + "_" + str(extraNNvalue) + '.pdf'
        plt.savefig(figure_name)
        #plt.show()
        plt.close()
    ...
    def plot_cell_transitions(self, cell_ix: int=0, alpha: float=0.1, alpha_neigh: float=0.2,
        ...
        plt.savefig("tSNE_cell_transitions.pdf")
        #plt.show()
        plt.close()
...
matrix = np.genfromtxt(sys.argv[5], delimiter=',', dtype=None) #main matrix mm10_pberghei_ercc_mouse_
col_gene_name_attribute = matrix[:,[0]]
...
for gene_num in range(1, len(col_gene_name_attribute)):
	gene = col_gene_name_attribute[gene_num]
	genes.append(str(gene, 'utf-8'))

matrix = np.delete(matrix, 0, 1)  #1 = col, 0 = row
row_patient_name_attribute = matrix[[0], :]
patients = []
...
for row in matrix:
	...
	for value in row: #remove the encoding added and ensure all the matrix values are integers
		rows.append(float(str(value, 'utf-8')))

	my_matrix.append(rows)
my_shape = np.array(my_matrix).shape
ncols = my_shape[0] #number of genes
nrows = my_shape[1] #number of cells
my_matrix = np.array(my_matrix).reshape(ncols,nrows)
print("Your data contains", ncols, "genes and", nrows, "samples")
#get the clinical complications as the clusters
metadata = np.genfromtxt(sys.argv[7], delimiter=',', dtype=None)
...
for meta in metadata:
    clusters.append(str(meta, 'utf-8'))

metadata = np.genfromtxt(sys.argv[8], delimiter=',', dtype=None)
metadatanames2 = list(metadata[:,0])
...
spliced_reads = np.genfromtxt(sys.argv[3], delimiter=',', dtype=None) 
...
unspliced_reads = np.genfromtxt(sys.argv[4], delimiter=',', dtype=None)
...
ambig_reads = np.genfromtxt(sys.argv[6], delimiter=',', dtype=None) #mm10_pberghei_ercc_mouse_ mm10_pchabaudi_ercc_mouse_
ambig_reads = np.delete(ambig_reads, 0, 1)  #1 = col, 0 = row
ambig_reads = np.delete(ambig_reads, 0, 0)
ambig = []

for row in ambig_reads:
	rows = []
	for value in row: #remove the encoding added and ensure all the matrix values are integers
		rows.append(float(0))
	ambig.append(rows)
ambig = np.array(ambig).reshape(ncols,nrows)

#feature_selection_practise(spl, 0.1, "variance", unspl, ambig, my_matrix, genes)
...
PCA_Folder = str(sys.argv[2] + sys.argv[10] + "Fate_Maps/PCA/" + str(sys.argv[30]) + "/")
UMAP_Folder = str(sys.argv[2] + sys.argv[10] + "Fate_Maps/UMAP_2D/" + str(sys.argv[30]) + "/")
tSNE_Folder = str(sys.argv[2] + sys.argv[10] + "Fate_Maps/tSNE/" + str(sys.argv[30]) + "/")
UMAP_3D_Folder = str(sys.argv[2] + sys.argv[10] + "Fate_Maps/UMAP_3D/" + str(sys.argv[30]) + "/")

VelocityValues_Folder = str(sys.argv[2] + sys.argv[10] + "Files/Velocity_Values/" + str(sys.argv[30])+ "/")
Predicted_Unspiced_Folder = str(sys.argv[2] + sys.argv[10] + "Files/Predicted_Unspiced/" + str(sys.argv[30]) + "/")
FutureSpliced_Folder = str(sys.argv[2] + sys.argv[10] + "Files/Future_Spliced/" + str(sys.argv[30]) + "/")

UMAP_3D_Delta_embedding_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Delta_Embedding/" + str(sys.argv[30]) + "/")
UMAP_3D_selfCC_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Self_Correlation_Coefficients/" + str(sys.argv[30]) + "/")
UMAP_3D_UnVecX_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Unitary_Vector_X/" + str(sys.argv[30]) + "/")
UMAP_3D_UnVecY_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Unitary_Vector_Y/" + str(sys.argv[30]) + "/")
UMAP_3D_UnVecZ_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Unitary_Vector_Z/" + str(sys.argv[30])+ "/")
UMAP_3D_Transition_Probability_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Transition_Probability/Sample_Level/" + str(sys.argv[30]) + "/")

UMAP_2D_Delta_embedding_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Delta_Embedding/" + str(sys.argv[30]) + "/")
UMAP_2D_selfCC_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Self_Correlation_Coefficients/" + str(sys.argv[30]) + "/")
UMAP_2D_UnVecX_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Unitary_Vector_X/" + str(sys.argv[30]) + "/")
UMAP_2D_UnVecY_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Unitary_Vector_Y/" + str(sys.argv[30]) + "/")
UMAP_2D_Transition_Probability_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Transition_Probability/Sample_Level/" + str(sys.argv[30]) + "/")

tSNE_Delta_embedding_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Delta_Embedding/" + str(sys.argv[30]) + "/")
tSNE_selfCC_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Self_Correlation_Coefficients/" + str(sys.argv[30]) + "/")
tSNE_UnVecX_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Unitary_Vector_X/" + str(sys.argv[30]) + "/")
tSNE_UnVecY_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Unitary_Vector_Y/" + str(sys.argv[30]) + "/")
tSNE_Transition_Probability_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Transition_Probability/Sample_Level/" + str(sys.argv[30]) + "/")

UMAP_2D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/")
UMAP_3D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/")
tSNE_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/")


CI_UMAP_2D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Confidence_Interval/" 
CI_UMAP_3D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Confidence_Interval/"
CI_tSNE_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Confidence_Interval/" 

BS_UMAP_2D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_2D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Bootstrapping/" 
BS_UMAP_3D_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/UMAP_3D/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Bootstrapping/"
BS_tSNE_GroupLevelTP_Folder = str(sys.argv[2] + sys.argv[10] + "Files/tSNE/Transition_Probability/Group_Level/" + str(sys.argv[30]) + "/") + "Bootstrapping/" 

#choose the colour scheme:
import matplotlib as mpl
import matplotlib.cm as cm
#norm = mpl.colors.Normalize(vmin=-20, vmax=10) #convert to greyscake
cmap = cm.Paired #https://matplotlib.org/users/colormaps.html
...
location = sys.argv[2] + sys.argv[10]
os.chdir(location)

##vlm.plot_fractions("plot.pdf")
...
if int(sys.argv[26]) == 3:
    make = [0,1,2]
if int(sys.argv[26]) == 2:
    make = [0,1]

#performs PCA: smoothing the data using kNN neighbours pooling
PCA_FileName1 = "_" + str(sys.argv[3]).split('_')[1] + "_PCA_Original"
PCA_FileName2 = "_" + str(sys.argv[3]).split('_')[1] + "_PCA_FS"
grouptype = sys.argv[16]
os.chdir(PCA_Folder)
vlm.perform_PCA(n_components=int(sys.argv[26])) #must be performed on normalised data, Barnes hut approximation
vlm.plot_pca(dim=make, make_gify=makegifs, Filename=PCA_FileName1, typegroup=grouptype) #plots 3D PCA, need to change
if sys.argv[9] == "PCAVar":
    location2 = sys.argv[2]
    os.chdir(location2)
    vlm.PCAVariationFS(n_pca_dims=int(sys.argv[26]), spliced_file=sys.argv[3], resuts_directory=PCA_Folder)
    os.chdir(PCA_Folder)
    vlm.perform_PCA(n_components=int(sys.argv[26])) #must be performed on normalised data, Barnes hut approximation
    vlm.plot_pca(dim=make, make_gify=makegifs, Filename=PCA_FileName2, typegroup=grouptype)
    print("PCA feature selection")
else:
    print("No PCA feature selection")
if sys.argv[11] == "CoorelationCoeff":
    os.chdir(location2)
    vlm.CorrelationCoefficentFS(clusters)
    os.chdir(PCA_Folder)
    vlm.perform_PCA(n_components=int(sys.argv[26])) #must be performed on normalised data, Barnes hut approximation
    vlm.plot_pca(dim=make, make_gify=makegifs, Filename="CCFS", typegroup=grouptype)
    print("Correlation coefficient feature selection")
else:
    print("No correlation coefficient feature selection")
...
metaloc = sys.argv[7]
metadataGroup2 = sys.argv[8]
R_Transition_Probability_Code = sys.argv[15]

#Embedding under UMAP
#UMAP
if str(sys.argv[24]) == "True":
    os.chdir(UMAP_Folder)
    vlm.basicUMAP(number_neighbous, make_gify=makegifs, distance=float(sys.argv[13]), metric_method=sys.argv[14], typegroup=grouptype, do3D=str(sys.argv[25]), UMAP_3D_loc=UMAP_3D_Folder)
    #Fit to 2 dimensions:
    #####do independent runs here
    for i in range(int(sys.argv[27]), int(sys.argv[28]), int(sys.argv[29])):
        os.chdir(UMAP_Folder)
        ...
        vlm.calculate_embedding_shiftUMAP(sigma_corr = 0.05, expression_scaling=scaling_to_DO, Myneighbours=number_neighbous, metadataToUse=metaloc, pathToTransitionProbabilityCode = R_Transition_Probability_Code, grouptype=grouptype, secondgroup=metadataGroup2, manyfeatures=str(sys.argv[18]), grouptype2=str(sys.argv[19]), Randomsampling=str(sys.argv[20]), extraNNvalue=i, DE_location=UMAP_2D_Delta_embedding_Folder, selfCC_folder=UMAP_2D_selfCC_Folder, VecX_location=UMAP_2D_UnVecX_Folder, VecY_loc=UMAP_2D_UnVecY_Folder, TP_folder=UMAP_2D_Transition_Probability_Folder, UMAP_2D_GroupLevelTP=UMAP_2D_GroupLevelTP_Folder)
        os.chdir(UMAP_Folder)
        vlm.UMAPwithArrows2D(UMAPembedding="UMAPembed", UMAP_Arrows="MY_ARROWS_umap", numberNeighb=number_neighbous, typegroup=grouptype, extraNNvalue=i)
        plt.close("all")
        #Fit to 3 dimensions
        if str(sys.argv[25]) == "True":
            ...
            vlm.calculate_embedding_shiftUMAP3D(sigma_corr = 0.05, expression_scaling=scaling_to_DO, Myneighbours=number_neighbous, metadataToUse=metaloc, pathToTransitionProbabilityCode = R_Transition_Probability_Code, grouptype=grouptype, secondgroup=metadataGroup2, manyfeatures=str(sys.argv[18]), grouptype2=str(sys.argv[19]), Randomsampling=str(sys.argv[20]), extraNNvalue=i, DE_location=UMAP_3D_Delta_embedding_Folder, selfCC_folder=UMAP_3D_selfCC_Folder, VecX_location=UMAP_3D_UnVecX_Folder, VecY_loc=UMAP_3D_UnVecY_Folder, VecZ_loc=UMAP_3D_UnVecZ_Folder, TP_folder=UMAP_3D_Transition_Probability_Folder, UMAP_3D_GroupLevelTP=UMAP_3D_GroupLevelTP_Folder)
            os.chdir(UMAP_3D_Folder)
            vlm.UMAPwithArrows3D(UMAPembedding="UMAPembed3d", UMAP_Arrows="MY_ARROWS_umap_3d", make_gify=makegifs, numberNeighb=number_neighbous, typegroup=grouptype, extraNNvalue=i)
            plt.close("all")
...
if  str(sys.argv[23]) == "True":
    ...
    ###indpeendent runs here
    vlm.ts = bh_tsne.fit_transform(vlm.pcs[:, :int(sys.argv[26])])
    for i in range(int(sys.argv[27]), int(sys.argv[28]), int(sys.argv[29])):
...
for Summaryfile in glob.glob(mylocytSNE):
    vlm.TransitionProbabilityScatterplots(filename=Summaryfile, make_gify=makegifs) #makes nice presentations of the transition probabilities
...
if str(sys.argv[18]) == "True":
    ...
    for Summaryfile in glob.glob(mylocytSNE2):
        if str(sys.argv[19]) == "integers": #and integers like time-course
            f=RunTransitionTypeEstimates(Summaryfile, tSNE_GroupLevelTP_Folder)
            plt.close("all")
...
if str(sys.argv[21]) == "True":
    ...
    for Summaryfile in glob.glob(mylocy3):
        j=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "FALSE", BS_UMAP_2D_GroupLevelTP_Folder, metaloc, UMAP_2D_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
        k=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "TRUE", BS_UMAP_2D_GroupLevelTP_Folder, metaloc, UMAP_2D_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
        l=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "FALSE", BS_UMAP_3D_GroupLevelTP_Folder, metaloc, UMAP_3D_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
        m=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "TRUE", BS_UMAP_3D_GroupLevelTP_Folder, metaloc, UMAP_3D_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
        n=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "FALSE", BS_tSNE_GroupLevelTP_Folder, metaloc, tSNE_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
        o=RunInternalBootStrappingTransitonProbabilities(Summaryfile, "TRUE", BS_tSNE_GroupLevelTP_Folder, metaloc, tSNE_GroupLevelTP_Folder, grouptype, int(sys.argv[22]))
        plt.close("all")
"""
