TransitionProbability <- function(filename, selfremoved, location, metaGroup, outputdir, grouptype, version, Randomsampling) {
  `%ni%` = Negate(`%in%`)
  ...
  for (value in (1:nrow(TPNN))) { #for each sample:
    ...
    randomDF_sumsPercent <- randomDF_sums*100
    RandomSamplingDF <- data.frame(randomDF_means$Group, randomDF_means$TransitionProbability, randomDF_medians$TransitionProbability, randomDF_sums$TransitionProbability, randomDF_sumsPercent$TransitionProbability)
    ...
    means <- aggregate( TransitionProbability ~ Group, TPNNnew, mean )     #get group mean and median
    medians <- aggregate( TransitionProbability ~ Group, TPNNnew, median )
    sums <- aggregate( TransitionProbability ~ Group, TPNNnew, sum )
    sumsAsPercent <- sums*100
    if (identical(means$Group, medians$Group) == TRUE) {
      values <- data.frame(means$Group, means$TransitionProbability, medians$TransitionProbability, sums$TransitionProbability, sumsAsPercent$TransitionProbability)
      colnames(values) <- c("Group", "Mean", "Median", "Sum", "Sum (%)") #put together
      valsvalues <- sort(values$Group)
  ...
  MeansTableMelt <- melt(MeansTable[,c(1,3:ncol(MeansTable))])
  ...
  #print(corrhm)
  #dev.off()
  SumsTableMelt <- melt(SumsTable[,c(1,3:ncol(SumsTable))])
  ...
  #print(corrhm3)
  #dev.off()
  MediansTableMelt <- melt(MediansTable[,c(1,3:ncol(MediansTable))])
  ...
  #dev.off()
  #Random Sampling:
  MeansTableMeltRS <- melt(MeansTableRS[,c(1,3:ncol(MeansTableRS))])
  ...
  if (Randomsampling == TRUE) {
    ...
    #print(corrhmRS)
    #dev.off()
    SumsTableMeltRS <- melt(SumsTableRS[,c(1,3:ncol(SumsTableRS))])
    ...
    #print(corrhm3RS)
    #dev.off()
    MediansTableMeltRS <- melt(MediansTableRS[,c(1,3:ncol(MediansTableRS))])
  ...
  threshold <- 1/nrow(TPNN) #get highest values, do they meet this threshold yes or no?
  if (typeof(res) == "list") {
    my_groups <- c()
    valuelist <- c()
    meetThreshold <- c()
  ...
  thresholdcolname <- paste0("Greater than 1/number of groups = ", toString(threshold), "?", sep="")
  if (selfremoved == "TRUE") {
  ...
  threshold <- 1/ncol(SumsTable2) #get highest values, do they meet this threshold yes or no?
  if (typeof(res) == "list") {
    my_groups <- c()
    valuelist <- c()
    meetThreshold <- c()
  ...
  thresholdcolnameRS <- paste0("Greater than 1/number of groups = ", toString(threshold), "?", sep="")
  if (selfremoved == "TRUE") {
  ...
  threshold <- 1/ncol(SumsTable2RS) #get highest values, do they meet this threshold yes or no?
  if (typeof(resRS) == "list") {
    my_groupsRS <- c()
    valuelistRS <- c()
    meetThresholdRS <- c()
